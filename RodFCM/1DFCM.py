...
from BoundaryCondition.PenaltyValueAlgorithm.StrongPenaltyAlgorithm import StrongPenaltyAlgorihtm
...
for element in elements:
    ...
    # Assign DOFs with PolynomialDegreeSorting
...
PenaltyAlgorithm = StrongPenaltyAlgorihtm(10e4)

InitialFix = StrongNodeDirichletBoundaryCondition([0, 0 ,0], 0, 1, PenaltyAlgorithm)

DisplacementEnd = StrongNodeDirichletBoundaryCondition([3, 0, 0], -0.02, 1, PenaltyAlgorithm)
...
for i, element in enumerate(elements):
    local_indices = LtoG[i]  # Local to global index mapping for this element
    local_displacements = solution[local_indices]  # Displacement solution for this element
    strains_at_gauss_points = []
    GP = GaussQuadratureCoordinates(NumberOfGaussPoints)
    dN = []
    # Loop through each Gauss point in the subdomain to calculate strain
    for j in range(len(GP)):
        dN.append(evalDerivOfShapeFunct(PolynomialDegree, GP[j]))
    strain_at_gauss_point = np.dot(dN, np.array(local_displacements).reshape(-1, 1))
    Strains.append(strain_at_gauss_point)

global_positions = np.linspace(0, 3, 32)
