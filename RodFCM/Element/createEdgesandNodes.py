...
def create_edges_on_line(nodes):
    ...
    for i in range(len(nodes)-1):
        ...
        edge = Edge(NodePair, 16, 1)
        edges.append(edge)
    return edges

def create_nodes_and_edges(elements, number_of_x_divisions):
    all_nodes = []
    all_edges = []
    new_elements = []
    ...
    for element in elements:
        ...
        nodes = create_nodes_on_line(element.local_coordinates[0], element.local_coordinates[1], number_of_x_divisions)
        ...
        global_increment = (element.global_coordinates[1] - element.global_coordinates[0]) / number_of_x_divisions
